...
function AddCustomer() {
    ...
    const [FormData, setFormData] = useState({
        id: Date.now(), CustomerName: '', CustomerEmail: '', CustomerMobile: '',
    })
    // useEffect(() => {
    ...
    function HandleSave() {
        if (id === "add") {
            ...
            customerservice.Add(updatedData);
    ...
    useEffect(() => {
        if (id !== 'add') {
            const existing = customerservice.GetById(Number(id));
            if (existing) {
            ...
            <Modal.Header className='modalheader'>
                <div className="titleconatiner">
                    <h1 className='headingstyle'>
                        {id === 'add' ? 'Customer Details' : 'EditDetails'}
                    </h1>
                </div>
